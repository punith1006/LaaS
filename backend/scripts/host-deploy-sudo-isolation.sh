...
if docker network ls 2>/dev/null | grep -q "laas-user-network"; then
    log_info "laas-user-network already exists"
    
    # Check ICC setting
    ICC=$(docker network inspect laas-user-network 2>/dev/null | grep -A5 "Options" | grep "icc" | awk -F'"' '{print $4}' || echo "")
    if [ "$ICC" = "false" ]; then
        ...
        if [[ "$response" =~ ^[Yy]$ ]]; then
            ...
            CONTAINERS_ON_NET=$(docker network inspect laas-user-network --format '{{range .Containers}}{{.Name}} {{end}}' 2>/dev/null || echo "")
            if [ -n "$CONTAINERS_ON_NET" ]; then
                log_error "Cannot recreate: containers using this network: $CONTAINERS_ON_NET"
            else
                docker network rm laas-user-network 2>/dev/null || true
                docker network create --driver bridge \
                    --opt "com.docker.network.bridge.enable_icc=false" \
                    laas-user-network
                log_success "Recreated laas-user-network with ICC disabled"
            fi
        fi
    fi
else
    docker network create --driver bridge \
        --opt "com.docker.network.bridge.enable_icc=false" \
        laas-user-network
    log_success "Created laas-user-network (inter-container communication disabled)"
fi
